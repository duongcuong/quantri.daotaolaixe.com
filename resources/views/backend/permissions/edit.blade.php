                ...
                <div class="card-body">
                    ...
                    <div class="form-group">
                        ...
                        <select name="status" id="status" class="form-control" required>
                            ...
                            <option value="0" {{ old('status', $admin->status) == '0' ? 'selected' : '' }}>Không hoạt động</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="card radius-15">
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <div class="thumbnail-container" onclick="document.getElementById('fileInput').click()" style="background-image: url('{{ getImageUpload($admin->thumbnail, 'admins', 'small') }}');">
                            <span class="placeholder" style="display: {{ $admin->thumbnail ? 'block' : 'none' }};">Chọn hình ảnh</span>
                            <input type="file" id="fileInput" accept="image/*" onchange="previewImage(event)" name="thumbnail">
                            <button class="delete-btn" style="display: {{ $admin->thumbnail ? 'block' : 'none' }}" onclick="deleteImage(event)">
                    ...
                    <div class="form-group">
                        ...
                        <select name="roles[]" id="roles" class="form-control single-select" multiple required>
                            @foreach ($roles as $role)
                                <option value="{{ $role->id }}" {{ (collect(old('roles', $admin->roles->pluck('id')->toArray()))->contains($role->id)) ? 'selected' : '' }}>{{ $role->name }}</option>
                            @endforeach
                        </select>
                    </div>
